...
CONTEXT_LAYERS="--context_layers 4"
...
if ! $PYTHON_CMD -c "from torch.utils.tensorboard import SummaryWriter" &>/dev/null; then
    echo -e "${YELLOW}TensorBoard not available. Installing...${NC}"
    $PYTHON_CMD -m pip install tensorboard
    if [ $? -ne 0 ]; then
...
echo -e "${GREEN}Context layers:${NC} ${CONTEXT_LAYERS#--context_layers }"
echo -e "${GREEN}Attention heads:${NC} ${ATTENTION_HEADS#--attention_heads }"
...
$PYTHON_CMD train_ravdess_advanced.py \
...
if [ $TRAINING_STATUS -eq 0 ]; then
    ...
    if [ -f "$OUTPUT_DIR/final_model.pt" ]; then
        ...
        $PYTHON_CMD ravdess_evaluate.py \
...
echo -e "${BLUE}$PYTHON_CMD ravdess_inference.py --model_path $OUTPUT_DIR/final_model.pt${NC}"
